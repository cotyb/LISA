...
from mininet.node import Host, UserSwitch, OVSKernelSwitch, Controller, RemoteController
import time
import random
...
class StanfordTopo( Topo ):
    ...
    def __init__( self ):
        ...
        # Wire up hosts
        host_id = len(self.all_switches) + 1
        for s in self.all_switches:
            # Edge ports
	    if s not in self.access_table.keys():
                self.access_table[s] = []
            for port in ports[s]:
                host = self.addHost( "h%s" % host_id )
		if host not in self.access_table[s]:
                    self.access_table[s].append(host)
    ...
    def create_links(self, links, ports):
		...
		self.addLink( node1="s%s" % dummy_switch_id, node2="s%s" %dpid, port1=1, port2=port)
                dummy_switch_port = 2
		    ...
		    self.addLink( node1="s%s" % dst_dpid, node2="s%s" %dummy_switch_id, port1=dst_port, port2 = dummy_switch_port)
		...
		self.addLink(node1="s%s" %dst_dpid, node2="s%s" %dpid, port1=dst_port,port2=port)
                ports[dst_dpid].discard(dst_port)
            ports[dpid].discard(port)
...
class StanfordMininet ( Mininet ):
	...
	self.topo.addLink(node1="s%s" %16, node2="s%s" %15,port1=4, port2=7)

def Lisa_tester(controller_ip, controller_port, dummy_controller_ip, dummy_controller_port):
    topo = StanfordTopo()
    dummyClass = lambda a: RemoteController( a, ip=dummy_controller_ip, port=dummy_controller_port)
    net = StanfordMininet(topo = topo, switch = OVSKernelSwitch, controller=dummyClass)
    net.start()
    num_switch = len(topo.all_switches)
    while True:
        switch1 = random.randrange(0,num_switch)
	src_sw = topo.all_switches[switch1]
        switch2 = random.randrange(0,num_switch)
        while switch2 == switch1:
            switch2 = random.randrange(0,num_switch)
        dst_sw = topo.all_switches[switch2]
        host_num1 = len(topo.access_table[src_sw])
	host_num2 = len(topo.access_table[dst_sw])
	#src_host_pos = random.randrange(0,host_num1)
	#dst_host_pos = random.randrange(0,host_num2)
	#src_host = topo.access_table[src_sw][src_host_pos]
	src_host = topo.access_table[src_sw][0]
	#dst_host = topo.access_table[dst_sw][dst_host_pos]
	dst_host = topo.access_table[dst_sw][0]
	net.ping([net.getNodeByName(src_host), net.getNodeByName(dst_host)])
	time.sleep(10)
...
if __name__ == '__main__':
    ...
    Mininet.init()
    Lisa_tester(gethostbyname(args.controller_name), args.controller_port, gethostbyname(args.dummy_controller_name), args.dummy_controller_port)
