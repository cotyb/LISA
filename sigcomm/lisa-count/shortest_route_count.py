...
class Shortest_Route(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        #files contain flow rules
        #self.files = os.listdir("flows")
    ...
    '''
    generate two ip that belong to the field of 10.0.0.1/24
    input: nothing
    return:
        lower_ip: int, the last eight bit
        upper_ip: int, the last eight bit
    '''
    def generate_count_ip_field(self, src_ip):
        lower_ip = random.randrange(1,src_ip)
        upper_ip = random.randrange(src_ip,241)
    ...
    def _packet_in_handler(self, ev):
        ...
        if isinstance(ip_pkt, ipv4.ipv4):
            ...
            if result:
                src_ip_last_eight = int(ip_src.split(".")[-1])
                wfile = open("result","a")
                print >> wfile, "statistics" 
                lower, upper = self.generate_count_ip_field(src_ip_last_eight)
                ...
                statistics = {}
                for switch in path:
                    statistics[switch] = [0, 0, 0]
                    count_need_add = 0
                    total_rules_num = 0
                    addition_rate = 0
                    file = "flows/s%s_flow" %switch
                    fp = open(file,"r")
                    rules = fp.readlines()
                    total_rules_num = len(rules)
                    for rule in rules:
                        rule = eval(rule)
                        if rule["src_ip"] in ip_set:
                            #count_need_add += 1
                            if rule["src_ip"] not in tmp_ip_set:
                                tmp_ip_set.append(rule["src_ip"])
                    count_need_add += (len(ip_set) - len(tmp_ip_set))
                    total_rules_num += count_need_add
                    addition_rate = 1.0 * count_need_add / total_rules_num
                    fp.close()
                    statistics[switch] = [count_need_add, total_rules_num, addition_rate]
                print >> wfile, "src_ip: %s, dst_ip: %s, path: %s, count_ip_range:%s--%s, %s" %(ip_src, ip_dst, path, lower_ip, upper_ip, statistics)
