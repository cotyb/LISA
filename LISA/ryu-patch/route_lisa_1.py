...
class Shortest_Route(app_manager.RyuApp):
    ...
    _CONTEXTS = {
        ...
        # "Middle_End": IR.middle_end.Middle_End,
    }
    ...
    def __init__(self, *args, **kwargs):
        ...
        # self.middle_end = kwargs["Middle_End"]
	...
	if information != None:
        ...
        for dummy_i in range(len(all_switches)-1):
            # result1 = self.dijkstra(self.lisa_graph,all_switches[dummy_i])          
            result = nx.shortest_path(self.G, all_switches[dummy_i], all_switches[dummy_i + 1])
            if result:
		#distance = result[0][all_switches[dummy_i]][all_switches[dummy_i+1]]
                split_list = result[1:]
                # split_list1 = result1[1][all_switches[dummy_i]][all_switches[dummy_i+1]]
    ...
    def random_generate_information(self):
        '''
    ...
    def has_loop(self, switch_dof, full_path, ip_dst):
        include_loop = True
        ...
        if len(full_path_set) == len(full_path):
            include_loop = False
            # get the port of the path exclude the last switch
	...
	#print "packet in"
	    ...
	    #self.route_lisa(ip_src,ip_dst,[{1:6}])
            ...
            if src_sw and dst_sw:
                result = nx.shortest_path(self.G, src_sw, dst_sw)
            #print "-----------------result---------------"
            #print result	
            #print "------------------graph-----------------"
            #print self.graph
            ...
            if result:
                ...
                print full_path
                has_loop, dof = self.has_loop(switch_dof, full_path, ip_dst)
                if has_loop:
